...
creditor IDs before sending the message to a particular accounting authority
node. (Also, the reverse translation should be applied for incoming
messages.)
...
When the interest rate is not zero (it can be positive or negative), an
...
*accounting authority node*. In this imaginary scenario, one of the currency
...
* Every new account receives an account identifier (the `account_id` text
  field). The *(debtor_id, account_id)* pair uniquely identifies each SMP
  account. To send money to a given account, the payer must know both the
  debtor ID (the currency), and the account identifier.
...
* When preparing a transfer, the payer has the option to request some amount
  ...
  amount. *The only purpose of locking* is to guarantee a successful commit
  in the future. Collecting several such guarantees, allows several prepared
  transfers to be committed atomically (all or nothing).
...
* When a transfer has been successfully committed, both the sender an the
  ...
  to the recipient, thus protecting him/her from being spammed with lots of
...
* The owner of one account is not the only entity that is able to initiate
  ...
  *coordinators*. In fact, the owner of the account is just a somewhat
  special "direct" type of coordinator. In the above diagram, this is
  expressed by the value of the `coordinator_type` text field.
...
first place. If every account that every currency holder creates starts out
with no money in it, where all the money originally comes from?

The answer is quite simple: **All the money comes from the debtor's
account.**
...
is that each currency issuer (aka debtor) creates a special account called
"the debtor's account". The debtor’s account is special in several ways:
...
being a relatively complex protocol, some details still remain veiled. You
can check [the protocol specification](/public/docs/protocol.pdf) for more
information.
